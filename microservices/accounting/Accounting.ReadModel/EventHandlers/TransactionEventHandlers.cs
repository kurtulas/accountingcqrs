...
namespace Accounting.ReadModel.EventHandlers
{
        ...
        ISubscribeSynchronousTo<TransactionAggregate, TransactionId, TransactionRegisterCompletedEvent>
    {
        ...
        public TransactionEventHandlers(ICommandBus commandBus,
            IReadModelStore<TransactionReadModel> transactionReadModelStore)
        {
            _commandBus = commandBus;
            _transactionReadModelStore = transactionReadModelStore;
        }
   
        public async Task HandleAsync(IDomainEvent<TransactionAggregate, TransactionId, TransactionRegisteredEvent> domainEvent,
            CancellationToken cancellationToken)
        {
            await _commandBus.PublishAsync(
                new RegisterTransactionCompleteCommand(domainEvent.AggregateEvent.Transaction),
                cancellationToken);
        }
        ...
        {
            //await _commandBus.PublishAsync(new RegisterCustomerCommand("ulas", "kurt",0 ),
            //     cancellationToken);
        }
